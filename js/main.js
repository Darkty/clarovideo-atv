...
var atvutils = ATVUtils = {
	...
	loadURLInternal: function(url, method, headers, body, loader) {
		...
		return xhr = me.makeRequest(url, method, headers, body, function(xml) {
			try {
				loader(proxy, xml);
			} catch(e) {
	...
	loadURL: function( options ) { //url, method, headers, body, processXML) {
		...
		return this.loadURLInternal(url, method, headers, body, function(proxy, xml) {
...
function decorateURLWithProxy(url) {
    ...
    if (deviceToken == null) {
        ...
        httpRequest.open("GET", 'http://mcbkairrulo.local/clarovideo-atv/js/objects/device-token.json');
...
function showTextEntry() {
    ...
    textEntry.image = 'http://mcbkairrulo.local/clarovideo-atv/icon/clarovideo.png'
...
function showContextMenu(itemId) {
	...
	{
		var ajax = new ATVUtils.Ajax({
			'url':'http://mcbkairrulo.local/clarovideo-atv/js/context-menu.xml',
			'success':function(xhr) {
